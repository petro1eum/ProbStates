...
$$H_4(p, e^{i\phi}) = H(p) + S_\phi(p, \phi)$$

где $H(p)$ — классическая энтропия, а $S_\phi(p, \phi)$ — энтропийный вклад фазы.

Энтропийный вклад фазы определяется через непрерывную энтропию распределения фазы:

$$S_\phi(p, \phi) = -\int_0^{2\pi} f_p(\theta, \phi)\log_2 f_p(\theta, \phi) d\theta$$
