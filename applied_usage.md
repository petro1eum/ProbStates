...
## Новые возможности (простыми словами)

### 1) Программируемый фьюжн: одна ручка κ вместо "магии" фаз
Задача: сделать так, чтобы объединение источников вело себя так, как вам нужно. Хотите как классика? Или чуть усиливать согласованные сигналы? Или, наоборот, сильнее отрубать дубли?

Идея простая: у нас есть ручка κ (каппа) — это «средняя согласованность». 
- κ = 0 — никакой магии: итог примерно как обычное сложение независимых вероятностей.
- κ < 0 — режим дедупликации: дубли режутся сильнее, итоги консервативнее.
- κ > 0 — режим усиления: согласие источников усиливается, итоги выше.
- κ* — специальное значение, при котором итог ровно совпадает с классическим OR по среднему (для тех, кто хочет строго соответствовать классике).

Как применить на практике (без формул):
```python
from probstates.calculus import rho_from_ps, kappa_for_classical_or, mc_expected_l4_quant_or_under_beta
import numpy as np

# У вас есть исторические оценки вероятностей по источникам (как список чисел) — это и есть ps
ps1 = [0.12, 0.10, 0.15, 0.14, 0.11]
ps2 = [0.18, 0.17, 0.20, 0.19, 0.16]

mu1, mu2 = float(np.mean(ps1)), float(np.mean(ps2))  # средние
rho1, rho2 = rho_from_ps(ps1), rho_from_ps(ps2)      # поправка на разброс (автоматически)

kappa_star = kappa_for_classical_or(mu1, mu2, rho1, rho2)  # «как классика по среднему»
print("κ* =", round(kappa_star, 3))

# Хотите усилить? Возьмите каппу ближе к +1, хотите сильнее дедупликацию — ближе к −1
# Проверяем, что получится по ожиданию (через быстрый Монте-Карло)
est = mc_expected_l4_quant_or_under_beta(8, 5, 6, 7, kappa=kappa_star, num_samples=20000)
print("Какой будет средний итог при κ*:", round(est, 4))
```
Совет: начните с κ*, посмотрите на стабильность и бизнес‑метрики, потом аккуратно подвигайте κ вверх/вниз.

### 2) Чувствительность и робастность (перед запуском в прод)
Просто и по делу:
- «Стабильно ли решение?» — добавьте шумы (`dephase`, `amp_damp`) и посмотрите, насколько меняется итог. Если сильно прыгает — либо углы выставлены слишком агрессивно, либо режим слишком «quant».
- «Кто влияет сильнее?» — меняйте входные p по одному на пару процентов и смотрите, как меняется итог — это «чувствительность». Большая чувствительность = место риска.

Рецепт:
```python
from probstates.coherence import dephase, amp_damp

baseline = smart_risk  # ваш итоговый PhaseState
for noise_level in [0.05, 0.15, 0.25]:
    noisy = amp_damp(dephase(baseline, sigma_phi=noise_level), alpha=noise_level/2)
    print(f"Шум={noise_level}: итог={round(noisy.probability,4)}")

# Мини-чек влияния одного источника
def influence_check(p_base, delta=0.02):
    lo = max(0.0, p_base - delta)
    hi = min(1.0, p_base + delta)
    return lo, hi
```
Правило: если при реалистичных шумах вывод меняется мало — конфигурация ок. Если меняется сильно — ослабьте режим (на ‘weight’/‘norm’) или рассинхроньте «подозрительные» пары (φ ближе к π).

### 3) «Вероятность вероятности»: когда p само по себе плавает
Иногда вы не уверены в самой оценке p (мало данных, сезонность). Тогда правильно учитывать «p как распределение», а не как одно число. Для этого — простой Монте‑Карло.
...
# У вас по источнику есть (условно) «α успехов» и «β неудач». Это даёт распределение на p.
a1,b1 = 12, 8   # источник 1
a2,b2 = 9,  7   # источник 2

N = 20000
ps1 = beta(a1,b1,size=N)
ps2 = beta(a2,b2,size=N)

# Считаем средний итог «или» на уровне 2 (классика)
classic = np.mean([(ProbabilisticBit(p1) | ProbabilisticBit(p2)).probability for p1,p2 in zip(ps1,ps2)])
print("Средний итог (классика):", round(float(classic),4))

# А теперь на уровне 4 (с фазой) — задаём простейшие фазы: согласие/конфликт
from probstates import PhaseState, set_phase_or_mode
set_phase_or_mode('weight')  # безопасный режим

def fuse_l4(p1,p2,phi1=0.0,phi2=np.pi):
    return (PhaseState(p1,phi1) | PhaseState(p2,phi2)).probability

fused = np.mean([fuse_l4(p1,p2) for p1,p2 in zip(ps1,ps2)])
print("Средний итог (уровень 4):", round(float(fused),4))
```
Зачем это нужно: вы видите не только один ответ, а «ожидаемый» ответ при вашей неопределённости по p. Это честнее и стабильнее, особенно при небольших выборках.
